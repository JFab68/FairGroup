...
exports.requireSubcommitteeLeadership = async (req, res, next) => {
  try {
    ...
    const subcommittee = await db.query('SELECT chair_id, vice_chair_id FROM subcommittees WHERE id = $1', [subcommitteeId]);
    ...
    if (chair_id === req.session.memberId || vice_chair_id === req.session.memberId) return next();
